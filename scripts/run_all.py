...
def lgb_fit(X, y, device="cpu"):
    ...
    X_tr, X_val = X_safe.iloc[:val_split], X_safe.iloc[val_split:]
    y_tr, y_val = y.iloc[:val_split],      y.iloc[val_split:]

    model = make_lgb_fast(X.columns, device=device, params_override=BEST_LGB_PARAMS)
